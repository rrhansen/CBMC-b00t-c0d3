...
int final_jump_to_rom_ext(rom_ext_manifest_t current_rom_ext_manifest) { // Returns a boolean value.
    __CPROVER_assume(current_rom_ext_manifest.image_code == &__some_func); //otherwise pointer errors
    ...
    __CPROVER_assert(rom_ext_entry == current_rom_ext_manifest.image_code, "PROPERTY 6: Correct entry point address.");
    rom_ext_entry();
...
/// Used for PROPERTY 8
/// </summary>
int __total_failure = 0;
int __boot_failed_executed = 0;

void boot_failed(boot_policy_t boot_policy) {
    __CPROVER_assert(__total_failure, "PROERTY 8: Executing Failure when all ROM EXT are invalid.");
    boot_policy.fail();
    __boot_failed_executed = 1;
}
...
/// Used for PROPERTY 7
/// </summary>
int __rom_ext_returned = 0;
int __boot_failed_rom_ext_terminated_executed = 0;
...
void PROOF_HARNESS() {
    ...
    {
        __CPROVER_assert(0, "Reachability check, should always \033[0;31mFAIL\033[0m");
        __CPROVER_postcondition(__boot_failed_executed, "PROPERTY 8: If all ROM EXT are invalid, we should have executed failure.");
    }
    else
    {
        __CPROVER_assert(0, "Reachability check, should always \033[0;31mFAIL\033[0m");
        __CPROVER_postcondition(__boot_failed_executed == 0, "PROPERTY 8: If all ROM EXT are not invalid, we should not have executed failure.");

    }

    // 11 reachability tests.
}
...
void mask_rom_boot(boot_policy_t boot_policy, rom_exts_manifests_t rom_exts_to_try) {
    ...
    for (int i = 0; i < rom_exts_to_try.size; i++)
    {
        ...
        __CPROVER_assert(__check_one == 0, "PROPERTY 7: 0 indicates it passed the test.");
        ...
        __CPROVER_assert(__check_two == 0, "PROPERTY 7: 0 indicates it passed the test.");
        ...
        __CPROVER_assert(__check_three == 0, "PROPERTY 7: 0 indicates it passed the test.");
